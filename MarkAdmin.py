...
class ImageView(RoledView):
    role = 'uploader'
    column_searchable_list = ['image']

    column_filters = (
        FilterPhrase(Image.image, "Название"),
    )
    ...
    def index(self):
        if request.method == 'POST':
            ...
            for f in file_obj:
                ...
                #im = file.open(file.stream)
                #w, h = im.size
                foto = Image(image=filename, width=0, height=0)
...
def CreateWallAdmin(app, db):
    ...
    admin.add_view(AdminView(Image, db.session, 'Картинки'))
